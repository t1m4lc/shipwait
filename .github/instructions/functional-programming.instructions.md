...
const withRetry = <T>(fn: () => Promise<T>, maxAttempts = 3): () => Promise<T> => {
  return async () => {
    let lastError: Error;
...
const ask = <E>(): Reader<E, E> => env => env;

const asks = <E, A>(f: (env: E) => A): Reader<E, A> => env => f(env);
...
const trampoline = <T>(fn: (...args: any[]) => T | (() => T | (() => T))): ((...args: any[]) => T) => {
  return (...args) => {
    let result = fn(...args);
...
const actionHandlers = {
  ...
  'UPDATE_ITEM': (state, action) => state.map(item => 
...
function* lazyFilter<T>(arr: T[], predicate: (item: T) => boolean): Generator<T> {
